...
export class OrdersController {
  ...
  async downloadPdf(@CurrentUser() user: AuthenticatedUser, @Param("id") id: string, @Res() res: Response) {
    const pdf = await this.orderPdfService.render(user.tenantId, id);
  ...
  update(@CurrentUser() user: AuthenticatedUser, @Param("id") id: string, @Body() dto: UpdateOrderDto) {
    return this.ordersService.update(user.tenantId, id, user.userId, dto);
  }
  ...
  confirm(@CurrentUser() user: AuthenticatedUser, @Param("id") id: string) {
    return this.ordersService.confirm(user.tenantId, id, user.userId);
  }
  ...
  cancel(@CurrentUser() user: AuthenticatedUser, @Param("id") id: string, @Body() dto: CancelOrderDto) {
    return this.ordersService.cancel(user.tenantId, id, user.userId, dto.reason);
  }
  ...
  duplicate(@CurrentUser() user: AuthenticatedUser, @Param("id") id: string) {
    return this.ordersService.duplicate(user.tenantId, id, user.userId);
  }
}
